...
[2026-04-09] risk_agent.py — kill switch priority order hardcoded (drawdown first), peak equity from trades table (reporter_agent not yet built), entry price from fresh yfinance fetch (cache_expiry_hours=0)
[2026-04-05] watchlist_agent.py: research_reports filtered by run_date column (not DATE(completed_at)) — robust against midnight boundary. Mixed sentiment → PROCEED (1 scorecard point). Both PROCEED and SKIP written to watchlist for audit trail. Partial scorecard max=20 (15 with earnings flag); threshold enforced by risk_agent on full scorecard.
[2026-04-05] screener_agent.py: INSERT OR REPLACE on UNIQUE(symbol, run_date) — emergency rescreen overwrites Monday run; most recent run is authoritative. regime_blocked still writes top5 with position_size_multiplier=0.0 so Watchlist Builder can read the list. get_nifty_universe_for_year fallback to fetch_nifty50_symbols() when year > 2023.
